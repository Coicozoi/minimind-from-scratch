...
from dataset.lm_dataset_r1 import GSM8KDataset
...
# Pre-compile regex patterns (module-level, avoid recompiling per call)
FMT_PATTERN = re.compile(r"<think>\s*(.+?)\s*</think>\s*<answer>\s*(.+?)\s*</answer>", re.DOTALL)
NUM_PATTERN = re.compile(r"[-+]?\d[\d,]*\.?\d*")


def calculate_rewards(prompts, responses, ground_truths):
    """
    Rule-based reward adapted to minimind chat template:
    <think> is in the prompt (auto-inserted), so completions start AFTER </think>.

      format:      +0.5 if completion contains </think>
      answer_tag:  +0.5 if text after </think> contains an <answer>NUM</answer>
      correctness: +2.0 if extracted number == ground_truth
      rep_penalty: -0.5 * rep_penalty(response)
    """
    ...
    for i in range(batch_size):
        ...
        for j in range(args.num_generations):
            ...
            # ==== format reward: has closing </think> ====
            ...
            # ==== answer_tag reward: has <answer>...</answer> ====
            tag_match = answer_tag_pattern.search(post_think)
            if tag_match:
                rewards[response_idx] += 0.5
                answer_text = tag_match.group(1).strip()
            else:
                # fallback: try to extract last number in the post-think text
                answer_text = post_think

            # ==== correctness reward ====
            if gt:
                nm = num_pattern.findall(answer_text)
                if nm:
                    # take the LAST number mentioned (usually the final answer)
                    pred = nm[-1].replace(",", "").strip()
                    try:
            ...
            rewards[response_idx] -= 0.5 * rep_penalty(response)

    return rewards, {
        "format_rate": fmt_count / max(total, 1),
        "correct_rate": correct_count / max(total, 1),
    }

def grpo_train_epoch(epoch, loader, iters, rollout_engine, ref_model, start_step=0, wandb=None, use_sglang=False):
    for step, batch in enumerate(loader, start=start_step + 1):
        ...
        rewards, reward_stats = calculate_rewards(prompts, completions, ground_truths)
        rewards = rewards.to(args.device)  # [B*num_gen]

        if args.debug_mode and is_main_process() and step % args.debug_interval == 0:
            for i in range(len(prompts)):
                Logger(f"[DEBUG] step={step}, sample[{i}], gt={ground_truths[i]}")
                Logger('-'*100)
                Logger(f"{'=' * 30} [DEBUG] sample[{i}] CONTEXT_BEGIN {'=' * 30}")
                Logger(prompts[i][-500:])  # tail only, prompts can be long
        ...
        if step % args.log_interval == 0 or step == iters:
            ...
            Logger(f'Epoch:[{epoch + 1}/{args.epochs}]({step}/{iters}), '
                   f'Reward: {avg_reward_val:.4f}, Fmt: {reward_stats["format_rate"]:.2f}, '
                   f'Correct: {reward_stats["correct_rate"]:.2f}, KL_ref: {kl_ref_val:.4f}, '
                   f'Adv Std: {advantages_std_val:.4f}, Actor Loss: {policy_loss_val:.4f}, '
                   f'Avg Len: {avg_len_val:.0f}, LR: {current_lr:.8f}')
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="MiniMind R1-style Rule-based GRPO")
    parser.add_argument("--save_dir", type=str, default="../out", help="模型保存目录")
    parser.add_argument('--save_weight', default='grpo_r1', type=str, help="保存权重的前缀名")
    parser.add_argument("--epochs", type=int, default=1, help="训练轮数")
    parser.add_argument("--batch_size", type=int, default=2, help="batch size")
    parser.add_argument("--learning_rate", type=float, default=1e-6, help="初始学习率(rule-based 可稍大)")
    ...
    parser.add_argument("--max_gen_len", type=int, default=384, help="生成的最大长度")
    parser.add_argument("--num_samples", type=int, default=1000, help="GSM8K训练样本数")
    parser.add_argument("--num_generations", type=int, default=4, help="每个prompt生成的样本数")
    parser.add_argument("--beta", type=float, default=0.04, help="KL惩罚系数(rule-based 可稍小)")
    ...
    parser.add_argument("--wandb_project", type=str, default="MiniMind-GRPO-R1", help="wandb项目名")
    ...
    parser.add_argument("--thinking_ratio", type=float, default=1.0, help="按概率开启thinking(R1 建议全开=1.0)")
    ...
    parser.add_argument("--sglang_shared_path", type=str, default="./sglang_ckpt_grpo_r1", help="SGLang共享存储路径")
    parser.add_argument("--max_steps", type=int, default=0, help="最大训练步数(0=跑满一个epoch)")
    ...
    if args.use_wandb and is_main_process():
        ...
        wandb_run_name = f"MiniMind-GRPO-R1-Epoch-{args.epochs}-BS-{args.batch_size}-LR-{args.learning_rate}"
    ...
    # ===== NOTE: no external reward model; reward is rule-based =====
    # Rollout 引擎
    ...
    # 数据(GSM8K)
    train_ds = GSM8KDataset(tokenizer, split="train", num_samples=args.num_samples, thinking_ratio=args.thinking_ratio)
    train_sampler = DistributedSampler(train_ds) if dist.is_initialized() else None
    optimizer = optim.AdamW(model.parameters(), lr=args.learning_rate)
    loader_for_count = DataLoader(train_ds, batch_size=args.batch_size, sampler=train_sampler)
    iters = len(loader_for_count)
    if args.max_steps > 0:
        iters = min(iters, args.max_steps)
    ...
    for epoch in range(start_epoch, args.epochs):
        ...
        if args.max_steps > 0:
            # truncate loader to max_steps
            from itertools import islice
            loader = list(islice(loader, args.max_steps))
        if skip > 0:
            Logger(f'Epoch [{epoch + 1}/{args.epochs}]: 跳过前{start_step}个step,从step {start_step + 1}开始')
            grpo_train_epoch(epoch, loader, len(loader) + skip, rollout_engine, ref_model, start_step, wandb, use_sglang = (args.rollout_engine == "sglang"))
        else:
            grpo_train_epoch(epoch, loader, len(loader), rollout_engine, ref_model, 0, wandb, use_sglang = (args.rollout_engine == "sglang"))
